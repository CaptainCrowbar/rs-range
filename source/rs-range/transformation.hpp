...
namespace RS::Range {
    ...
    public Detail::FlexibleIterator<MapHeadTailIterator<Range, UnaryFunction1, UnaryFunction2>,
        std::common_type_t<std::invoke_result_t<UnaryFunction1, TL::RangeValue<Range>>,
            std::invoke_result_t<UnaryFunction2, TL::RangeValue<Range>>>> {
    public:
        using underlying_iterator = TL::RangeIterator<const Range>;
        using iterator_category = typename std::iterator_traits<underlying_iterator>::iterator_category;
        using value_type = std::common_type_t<std::invoke_result_t<UnaryFunction1, TL::RangeValue<Range>>,
            std::invoke_result_t<UnaryFunction2, TL::RangeValue<Range>>>;
        using predicate_type = std::function<bool(const value_type&)>;
        using function_type = std::function<value_type(const TL::RangeValue<Range>&)>;
        MapHeadTailIterator() = default;
        MapHeadTailIterator(underlying_iterator i, UnaryFunction1 f1, UnaryFunction2 f2):
            iter_(i), fun1_(f1), fun2_(f2), value_(), ok_(false), tail_(false) {}
        ...
        MapHeadTailIterator& operator++() { ++iter_; ok_ = false; return *this; }
        MapHeadTailIterator& operator--() { --iter_; ok_ = false; return *this; }
        MapHeadTailIterator& operator+=(ptrdiff_t rhs) { iter_ += rhs; ok_ = false; return *this; }
        ptrdiff_t operator-(const MapHeadTailIterator& rhs) const { return iter_ - rhs.iter_; }
        bool operator==(const MapHeadTailIterator& rhs) const noexcept { return iter_ == rhs.iter_; }
    private:
        underlying_iterator iter_;
        function_type fun1_;
        function_type fun2_;
    ...
    template <typename Range, typename UnaryFunction1, typename UnaryFunction2>
    TL::Irange<MapHeadTailIterator<Range, UnaryFunction1, UnaryFunction2>> operator>>(const Range& lhs, MapHeadTailObject<UnaryFunction1, UnaryFunction2> rhs) {
        auto r = Detail::as_range(lhs);
        return {{r.begin(), rhs.fun1, rhs.fun2}, {r.end(), rhs.fun1, rhs.fun2}};
    }
    ...
    inline MapHeadTailObject<UnaryFunction1, Detail::IdentityFunction> map_head_tail(UnaryFunction1 f1, std::nullptr_t = nullptr) {
        return {f1, {}};
    }

    template <typename UnaryFunction2>
    inline MapHeadTailObject<Detail::IdentityFunction, UnaryFunction2> map_head_tail(std::nullptr_t, UnaryFunction2 f2) {
        return {{}, f2};
    }
